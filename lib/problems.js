...
// console.log(lucasNumberMemo(4)) //should be 7
...
function minChange(coins, amount, memo = {}) {
    ...
    //loop through all of coins
    //if coin is less than amount
    //push into coins array
    //then push result of minchange(coins, amount - coin, memo)
    let numberOfCoins = [];
    for(let i = 0; i < coins.length; i++) {
        if(coins[i] <= amount) {
            numberOfCoins.push(minChange(coins, amount - coins[i], memo) + 1)
            console.log(numberOfCoins);
        }
    }
    memo[amount] = Math.min(...numberOfCoins);
    return memo[amount];
